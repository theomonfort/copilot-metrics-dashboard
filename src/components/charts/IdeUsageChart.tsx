...
export function IdeUsageChart({ data }: IdeUsageChartProps) {
  ...
  for (const day of data) {
    for (const ide of day.totals_by_ide ?? []) {
      ideMap.set(ide.ide, (ideMap.get(ide.ide) ?? 0) + ide.code_generation_activity_count);
    }
  }

  const chartData = Array.from(ideMap.entries())
